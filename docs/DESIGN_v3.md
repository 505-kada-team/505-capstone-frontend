...
**Warna badge kategori (`ingredient` vs `packaging`) — ditambahkan, beda konsep dari status:**

Kategori itu taksonomi tetap (bukan kondisi yang berubah), jadi TIDAK pakai warna semantik (destructive/success/warning). Reuse token yang sudah ada, jangan bikin hex baru:
- `ingredient` → badge pakai token `secondary` (background `#E6D5C3` light / `#2D241E` dark, ikut mapping tabel di atas)
- `packaging` → badge pakai token `neutral` skala terang (background abu netral, bukan turunan Primary/Secondary/Tertiary)

## 2b. Catatan Implementasi dari Mockup Stitch (Ditambahkan)

- Warna & font yang tampil di screenshot/mockup Stitch **tidak bisa dipercaya sebagai referensi hex final** — walau kelihatan mirip di mata, selalu override paksa ke hex/token di Section 2 dan font di Section 3, jangan "eyeball" dari gambar.
- Berlaku khusus untuk tombol destructive vs primary/tertiary yang huenya berdekatan (`#C4441F` vs `#F97316`) — pastikan Antigravity pakai token yang benar per konteks (hapus/destructive vs CTA utama), bukan warna hasil generate Stitch yang belum tentu presisi.
- Recipe **tidak punya status field** (`Draft`/`Active` di mockup Recipes adalah buatan Stitch, bukan bagian dari data model) — abaikan sepenuhnya. Semua recipe pakai action link yang sama ("Detail"), tidak ada varian "Edit Draft".
...
- Kontrol filter (dipakai di banyak halaman lewat komponen shared) label tombolnya **"Filter"** di semua halaman — bukan "More Filters" di sebagian halaman
...
- **Em-dash (`—`) untuk null value** (`quantityTotal: null`, `expired: null` pada kategori packaging): warna teks `muted`, font tetap Geist Mono mengikuti kolom aslinya (biar tidak "loncat" visual).
