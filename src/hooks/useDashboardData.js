...
const useDashboardData = () => {
  ...
  const chartData = useMemo(() => {
    const hourlyTrends = summaryData?.hourlyTrends || [];
  ...
  const hourlyTrends = summaryData?.hourlyTrends || [];
  const menuBreakdown = summaryData?.menuBreakdown || [];
  ...
  const averageRevenuePerCup = useMemo(() => {
    ...
    const cups = Number(summaryData?.totalUnitsSold || 0);

    return cups > 0 ? revenue / cups : 0;
  }, [summaryData]);
  ...
  const salesConcentration = useMemo(() => {
    ...
    const concentratedRevenue = topHours.reduce(
  ...
  const topMenuRevenueShare = useMemo(() => {
    ...
    if (totalRevenue <= 0 || menuBreakdown.length === 0) {
      return {
        menu: null,
        percentage: 0,
      };
    ...
    return {
      menu: topMenu,
      percentage:
        (Number(topMenu.revenue || 0) / totalRevenue) * 100,
    };
  ...
  const kpi = {
    ...
    totalCupsSold: Number(summaryData?.totalUnitsSold || 0),
  };
