...
export default function AdminLayout() {
  ...
  const [isProductionPlanOpen, setIsProductionPlanOpen] =
    useState(isProdPlanActive);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex">
      {/* SIDEBAR (Fixed Left) */}
      <aside
        className={`fixed left-0 top-0 bottom-0 bg-sidebar border-r border-sidebar-border flex flex-col z-20 transition-[width] duration-300 ease-in-out overflow-hidden ${
          isSidebarOpen ? "w-64" : "w-20"
        }`}
      >
        {/* Brand Header */}
        <div
          className={`h-16 flex items-center border-b border-sidebar-border transition-[padding] duration-300 ease-in-out ${
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div
              className={`flex-1 whitespace-nowrap transition-all duration-300 ease-in-out ${isSidebarOpen ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4 hidden"}`}
            >
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-3 space-y-1 overflow-x-hidden custom-scrollbar">
          ...
          >
            ...
            <span
              className={`whitespace-nowrap transition-opacity duration-300 ${isSidebarOpen ? "opacity-100" : "opacity-0 hidden"}`}
            >
              Dashboard
            </span>
          </NavLink>
          ...
          <NavLink
            to="/admin/inventory"
            className={navLinkStyle}
            title={!isSidebarOpen ? "Inventory" : undefined}
          >
          ...
          <NavLink
            to="/admin/recipes"
            className={navLinkStyle}
            title={!isSidebarOpen ? "Recipes" : undefined}
          >
          ...
          <div>
            <button
              ...
              className={`w-full flex items-center rounded-md py-2 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${
                isProductionPlanOpen || isProdPlanActive
            ...
            >
              <div className="flex items-center gap-3">
                <Factory
                  size={20}
                  className={`shrink-0 ${isProductionPlanOpen || isProdPlanActive ? "text-accent" : ""}`}
                />
                <span
                  className={`font-medium whitespace-nowrap transition-opacity duration-300 ${isSidebarOpen ? "opacity-100" : "opacity-0 hidden"} ${isProductionPlanOpen || isProdPlanActive ? "font-medium" : ""}`}
                >
                  Production Plan
                </span>
              </div>
            ...
            {isSidebarOpen && (
              <div
                className={`overflow-hidden transition-all duration-300 ease-in-out ${isProductionPlanOpen ? "max-h-60 mt-1" : "max-h-0 mt-0"}`}
              >
                <div className="space-y-1">
                  <NavLink
                    to="/admin/production-plan/draft"
                    className={subNavLinkStyle}
                  >
                    <span className="whitespace-nowrap">Draft Plan</span>
                  </NavLink>
                  <NavLink
                    to="/admin/production-plan/active"
                    className={subNavLinkStyle}
                  >
                    <span className="whitespace-nowrap">Active Plan</span>
                  </NavLink>
                  <NavLink
                    to="/admin/production-plan/report"
                    className={subNavLinkStyle}
                  >
      ...
      {/* MAIN CONTENT AREA */}
      <div
        className={`flex-1 flex flex-col min-h-screen transition-[margin] duration-300 ease-in-out ${isSidebarOpen ? "ml-64" : "ml-20"}`}
      >
        {/* HEADER (Fixed Top) */}
        <header
          className={`h-16 fixed top-0 right-0 bg-background border-b border-border z-10 flex items-center justify-between px-6 transition-[left] duration-300 ease-in-out ${isSidebarOpen ? "left-64" : "left-20"}`}
        >
          ...
          <div className="flex items-center gap-4">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger className="flex items-center gap-2 outline-none focus-visible:ring-2 focus-visible:ring-ring rounded-full">
                <div className="size-8 rounded-full bg-secondary flex items-center justify-center text-secondary-foreground font-semibold font-mono">
        ...
        {/* OUTLET (Page Content) */}
        <main className="flex-1 mt-16 p-6 h-[calc(100vh-4rem)] overflow-y-auto overflow-x-hidden">
          <Outlet />
        </main>
      </div>
      <ChatWidget />
