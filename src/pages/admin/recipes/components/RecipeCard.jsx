  ...
  return (
    <Card className="group overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 flex flex-col h-full">
      ...
      <div className="p-2.5 flex flex-col flex-1">
        <h3 className="font-heading font-semibold text-sm text-foreground line-clamp-1 mb-0.5">{recipe.name}</h3>

        <p className="text-[11px] text-muted-foreground mb-2 flex-1 line-clamp-1">
          {recipe.totalIngredients} bahan
          {recipe.costComplete && typeof recipe.marginPercentage === 'number' ? ` · margin ${recipe.marginPercentage.toFixed(0)}%` : ''}
        </p>
        ...
        <div className="flex items-end justify-between mt-auto gap-1">
          <div className="min-w-0">
            ...
            <p className="font-mono text-xs text-foreground truncate">{recipe.sellingPrice != null ? formatCurrency(recipe.sellingPrice) : 'Rp -'}</p>
          </div>

          {recipe.status === 'active' && (
            <button onClick={onDetail} className="shrink-0 text-xs font-medium text-accent hover:text-accent/80 transition-colors flex items-center gap-0.5">
              Detail
