...
import { addSubInventory } from '@/services/api';
import { addSubInventorySchema } from '@/schemas/inventorySchema';

export default function AddBatchModal({ open, onClose, parentInventory, onSuccess }) {
  ...
  const {
    ...
    defaultValues: { 
      ...
      nameResponsible: 'Admin', // Default PIC
    },
  });
  ...
  useEffect(() => {
    if (open) {
      ...
      setValue('nameResponsible', 'Admin');
      if (isPackaging) {
        setValue('expired', null);
      }
    }
  }, [open, reset, isPackaging, setValue]);

  const onSubmit = async (data) => {
    // Manual conditional validation for 'expired' if category is 'ingredients'
    if (parentInventory?.category === 'ingredients' && !data.expired) {
      setError('expired', { message: 'Kadaluarsa wajib diisi untuk kategori ingredients' });
      return;
    }
    ...
    setServerError('');
    try {
      await addSubInventory(parentInventory._id, data);
      onSuccess?.();
      onClose();
    } catch (err) {
      const msg = err?.response?.data?.message ?? 'Gagal menyimpan batch. Coba lagi.';
      setServerError(msg);
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => { if (!isOpen) onClose(); }}>
      <DialogContent className="max-w-[425px] p-6" id="add-batch-dialog">
        <DialogHeader>
          <DialogTitle className="font-heading text-xl font-semibold">Add New Inventory Batch</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 pt-2">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              value={parentInventory.nameInventory}
              disabled
              className="bg-muted text-muted-foreground"
            />
            <p className="text-xs text-muted-foreground flex items-center gap-1 mt-1">
              <span className="inline-block w-3 h-3 rounded-full border border-current opacity-50 flex items-center justify-center text-[8px]">i</span>
              Kategori: {parentInventory.category === 'packaging' ? 'Packaging' : 'Ingredient'}
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5 relative">
              ...
              <div className="relative">
                ...
                <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                  <span className="text-sm text-muted-foreground font-mono">{parentInventory.unit}</span>
                </div>
              </div>
              {errors.quantity && (
            ...
            <div className="space-y-1.5 relative">
              ...
              {errors.costPrices && (
                <p className="text-xs text-destructive">{errors.costPrices.message}</p>
              )}
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            <Label htmlFor="batch-expired" className="text-sm font-medium">
              Kadaluarsa {parentInventory.category === 'ingredients' && <span className="text-destructive">*</span>}
            </Label>
            <Input
              id="batch-expired"
              type="date"
            ...
            />
            {isPackaging && (
              <p className="text-xs text-muted-foreground italic">Item packaging tidak memerlukan tanggal kadaluarsa.</p>
            )}
            {errors.expired && !isPackaging && (
              <p className="text-xs text-destructive">{errors.expired.message}</p>
            )}
          </div>
