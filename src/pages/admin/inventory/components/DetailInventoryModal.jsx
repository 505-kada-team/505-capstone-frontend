...
// BERUBAH: parentInventory sekarang datang dari mapInventoryDetail(), jadi
// field-nya `id`, `name`, `category`, `unit` — bukan `_id`/`nameInventory`.
export default function AddBatchModal({
  ...
  const {
    ...
    // BERUBAH: nameResponsible dihapus — backend gak punya field ini sama
    // sekali (lihat SubInventory model), jadi gak ada gunanya dikirim.
    defaultValues: {
      quantity: "",
      costPrices: "",
      expired: "",
  ...
  const onSubmit = async (data) => {
    // Validasi manual: expired wajib diisi kalau kategori ingredients.
    // Ini tetap dobel sama guard di backend (service.addSubInventory),
