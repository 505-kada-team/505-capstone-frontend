...
export default function ActiveMenuDetailModal({
  ...
  // Helper: calculate weighted average cost from plan's committed ingredients
  const getWeightedAvgCost = (inventoryId) => {
    const item = plan?.committedIngredients?.find(
      (ci) => ci.inventoryId === inventoryId
    );
    ...
    const totalCost = item.batches.reduce(
      (sum, b) => sum + (b.quantityUsed || 0) * (b.costPriceUsed || 0),
      0
    );
