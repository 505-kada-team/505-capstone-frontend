...
import { getPlanDetail, getPlanReportList, getSellingHistory } from "@/services/api";
import { formatCurrency } from "@/lib/formatCurrency";
import StatusBadge from "@/components/shared/StatusBadge";
import { ClipboardList, TriangleAlert, CheckCircle2, ShieldAlert } from "lucide-react";
import { toast } from "sonner";

export default function PlanHistoryDetailModal({
  isOpen,
  onClose,
  planId,
}) {
    ...
    if (isOpen && planId) {
      const fetchPlanData = async () => {
        ...
        try {
          ...
          toast.error(err?.response?.data?.message || err?.message || "Failed to load plan details");
          onClose();
        } finally {
          setIsLoading(false);
        }
      ...
      const fetchReports = async () => {
        ...
        try {
          const res = await getPlanReportList({ planId });
          if (res.data?.success) {
  ...
  const getMenuSalesAnalysis = (menu) => {
    const normalPrice = menu.frozenSellingPrice || menu.effectiveSellingPrice || 25000;
    ...
    const normalQty = normalSales.reduce((sum, s) => sum + (s.quantitySold || 0), 0);
    const promoQty = promoSales.reduce((sum, s) => sum + (s.quantitySold || 0), 0);
    
    const discountedPrice = menu.discount?.discountPercentage > 0 
      ? menu.discount.discountedPrice 
      : null;
      
    const normalRev = normalQty * normalPrice;
    const promoRev = promoSales.reduce(
      (sum, s) => sum + (s.quantitySold || 0) * (s.priceUsed || discountedPrice || normalPrice),
      0
    );
  ...
  const getRemainingStatusList = () => {
    return menus.map((m) => {
      const remaining = (m.quantityPlanned || 0) - (m.soldQuantity || 0) - (m.lossQuantity || 0);
      return {
        name: m.name,
        remaining,
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-full sm:max-w-4xl mx-auto max-h-[90vh] overflow-y-auto">
        {isLoading || !plan ? (
          ...
          <>
            <DialogHeader>
              <DialogTitle className="text-lg font-bold text-foreground font-heading flex items-center justify-between">
                <span className="flex items-center gap-2">
                  <ClipboardList className="w-5 h-5 text-[#F97316]" />
                  {plan.name}
                </span>
                <span className="text-sm capitalize animate-pulse-slow">
                  <StatusBadge variant={plan.status === 'completed' ? 'completed' : 'stopped'} />
                </span>
              </DialogTitle>
            </DialogHeader>
            ...
            <div className="border-t border-border pt-4 mt-2 space-y-6">
              <div className="text-xs text-muted-foreground font-medium">
                Plan Period:{" "}
                <span className="text-foreground">{formatPeriod(plan.startDate, plan.endDate)}</span>
              </div>
              <div>
                  ...
                  <table className="w-full text-sm text-left">
                    <thead className="bg-muted text-muted-foreground text-xs uppercase">
                      <tr>
                        ...
                        <th className="px-4 py-2.5 font-medium text-right">Planned (pcs)</th>
                        <th className="px-4 py-2.5 font-medium text-right">Price</th>
                        <th className="px-4 py-2.5 font-medium text-right">Est. Revenue</th>
                        <th className="px-4 py-2.5 font-medium text-right">Sold (pcs)</th>
                        <th className="px-4 py-2.5 font-medium text-right">Actual Revenue</th>
                        <th className="px-4 py-2.5 font-medium text-right">Variance</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y font-normal">
                      {menusWithAnalysis.map((m) => {
                        const estRev = (m.quantityPlanned || 0) * m.analysis.normalPrice;
                        const actRev = m.analysis.totalActualRevenue;
                        const variance = actRev - estRev;

                        return (
                          <tr key={m.menuId} className="bg-background hover:bg-muted/10">
                            ...
                            <td className="px-4 py-3.5 text-right font-mono text-xs">
                              <div>{formatCurrency(m.analysis.normalPrice)}</div>
                              {m.analysis.discountedPrice && (
                                <div className="text-[10px] text-[#F97316] font-semibold mt-0.5">
                                  Promo: {formatCurrency(m.analysis.discountedPrice)}
                                </div>
                              )}
                            </td>
                            <td className="px-4 py-3.5 text-right font-mono text-xs">
                            ...
                            <td className="px-4 py-3.5 text-right font-mono text-xs">
                              ...
                              {m.analysis.discountedPrice && (
                                <div className="text-[10px] text-muted-foreground mt-0.5">
                                  ({m.analysis.normalQty} normal / {m.analysis.promoQty} promo)
                                </div>
                              )}
                            </td>
                            <td className="px-4 py-3.5 text-right font-mono text-xs text-foreground font-semibold">
                              <div>{formatCurrency(actRev)}</div>
                              {m.analysis.discountedPrice && (
                                <div className="text-[10px] text-green-700 font-normal mt-0.5">
                                  (Promo: {formatCurrency(m.analysis.promoRev)})
                                </div>
                              )}
                            </td>
                            <td
                              className={`px-4 py-3.5 text-right font-mono text-xs font-semibold ${
                                variance < 0 ? "text-[#C4441F]" : "text-green-700"
                              }`}
                            >
                              {variance < 0 ? "-" : ""}
                ...
                {isLoadingReports ? (
                  ...
                  <div className="border rounded-lg overflow-hidden">
                    <table className="w-full text-sm text-left">
                      <thead className="bg-muted text-muted-foreground text-xs uppercase">
                        <tr>
                          <th className="px-4 py-2 font-medium">Incident Time</th>
                          <th className="px-4 py-2 font-medium">Type & Item</th>
                          <th className="px-4 py-2 font-medium text-right">Qty Lost</th>
                          <th className="px-4 py-2 font-medium text-center">Status</th>
                          <th className="px-4 py-2 font-medium">Admin Action / Resolution</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y font-normal">
                        {reports.map((rep) => {
                          ...
                          return (
                            <tr key={rep._id} className="bg-background hover:bg-muted/10 text-xs">
                                    ...
                                    <div className="flex flex-col text-green-700">
                                      ...
                                      {rep.category === "ingredient" ? (
                                        rep.replacementDeducted ? (
                                          <span className="text-[10px] text-muted-foreground font-mono">
                                            Replaced: {rep.replacementQuantity} units (Cost:{" "}
                                        ...
                                        <span className="text-[10px] text-muted-foreground">
                                          Est. Cost Loss: {formatCurrency(rep.valuation?.costLoss)}
                                        </span>
                                      )}
                                    </div>
                                  )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2 border-t border-border">
                    ...
                    <div className="space-y-1.5 pl-1">
                      {remainingStatusList.map((m, idx) => (
                        <div key={idx} className="flex justify-between items-center text-xs">
                          ...
                          >
                            {m.isSoldOut ? "Sold Out" : `${m.remaining} pcs remaining`}
                          </span>
                        </div>
                      ))}
                    </div>
                ...
                <div className="border border-border bg-secondary/10 dark:bg-muted/30 rounded-lg p-4 space-y-3">
                  ...
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-muted-foreground font-medium">Estimated Revenue</span>
                    <span className="font-mono font-bold text-foreground">
                      {formatCurrency(totalEstimatedRevenue)}
                    </span>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-muted-foreground font-medium">Actual Revenue</span>
                  ...
                  <div className="flex justify-between items-center text-sm border-t border-dashed border-border pt-2 font-bold">
                    ...
                    <span
                      className={`font-mono ${
                        totalRevenueVariance < 0 ? "text-[#C4441F]" : "text-green-700"
                      }`}
                    >
            ...
            <DialogFooter className="mt-6">
              <Button variant="outline" className="h-9 px-4 font-semibold shadow-xs" onClick={onClose}>
                Close
              </Button>
            </DialogFooter>
          </>
